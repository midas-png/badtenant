...
const SearchBar = ({ nameLike, limit, page, role, ratingRange, dateSortType, onChange }) => {
  ...
  const sortToggle = (sortType, setSortType) => {
    ...
    fetchFiltered(nameLike, limit, page, role, ratingRange, dateSortType).then((data) => advertisement.setAdvertisement(data));
  };
  ...
  return (
    <BarWrapper>
      <SearchInputButtonWrapper>
        <SearchBarWrapper>
          <SearchInput onChange={onChange} onKeyDown={(e) => handleInputEnter(e)} />
        </SearchBarWrapper>
