  ...
  const sendOffer = () => {
    try {
      createOffer(
        ...
        .catch(() => toast.error('Oops! Something went wrong. Try again later.'));
    } catch(e) {
            ...
            <ModalWrapper>
              ...
              <ModalDateWrapper>
                ...
                {user.user.role === advertisementInfo.role && (
                  <WarningWrapper>
                    <WarningIcon />
                    <WarningText>You both are {user.user.role === 'TENANT' ? 'tenants': 'landlords'}.<br/>Remember this when sending an offer!</WarningText>
                  </WarningWrapper>
                )}
                <TextAreaWrapper>
                  <TextArea
